...
// rgb with alpha to hex
function RGBAtoHex(r, g, b, a) {
    return "#" + (1 << 24 | r << 16 | g << 8 | b).toString(16).slice(1) + (1 << 8 | a).toString(16).slice(1);
}
...
Warps.prototype.add_pair = function (which, x, y) {
    ...
    if (meshGradient) {
        ...
        meshGradient.maybeSaveStateChanged();
    }
};
...
function init() {
    ...
    const exportbtn = document.querySelector("#exportFrame");
...
function updateColors() {
    ...
    const colorsElems = colorsWrapper.querySelectorAll(".color");
    colorsElems.forEach((color, index) => {
        const picker = color.querySelector("input");
        const colorIndex = picker.getAttribute('name');
        picker.value = colors[colorIndex];
...
function initColorPickerEvents() {
    ...
    colorsElems.forEach((color, index) => {
        ...
        picker.addEventListener("input", () => {
            changeColor(colorIndex, picker.value);
            meshGradient.hasUnsavedChanges = true;
            meshGradient.maybeSaveStateChanged();
        });
    })
...
class MeshGradientTimeline {
    ...
    hasTimelineUnsavedChanges = false;
    _saveStateLast = false;
    _timelineSaveStateLast = false;
    ...
    init() {
        ...
        document.documentElement.style.setProperty('--frame-transition-speed', `${this.transitionDuration}ms`);
    }
    ...
    addFrame() {
        ...
        // Add frame to timeline variable
        this.frames.push(frame);

        // Set the position index of the frame in the timeline.
        frame.setPositionIndex(this.frames.length - 1);
    ...
    deleteFrame() {
        ...
        editor.deleteFrame(this.frames[this.currentFrame]);
    ...
    updateURL() {
        const stateData = this.getSaveStateData();      
        ...
        this.hasTimelineUnsavedChanges = false;
    }
    ...
    loadURLState() {
        ...
        stateData.frames.forEach((frame, index) => {
            ...
            meshGradientFrame.setPositionIndex(index);
    ...
    transitionToFrame(frame, speed) {
        ...
        let tmpSpeed;
        if (speed) {
            tmpSpeed = JSON.parse(JSON.stringify(this.transitionDuration));
        ...
        setTimeout(() => {
            ...
            if (speed) {
                this.transitionDuration = tmpSpeed;
            }
    ...
    playTimeline() {
        ...
        for (let i = 0; i < this.frames.length; i++) {
            setTimeout(() => {
                ...
                const beforeFrameEvent = new CustomEvent('beforeFrameTransition', { detail: { saveState: this.hasUnsavedChanges } });
                window.dispatchEvent(beforeFrameEvent);

                // at 50ms, transition to next frame
                setTimeout(() => {
                    this.transitionToFrame(i);
                }, 50);

                // after transition duration, fire end event
                setTimeout(() => {
                    const afterFrameEvent = new CustomEvent('afterFrameTransition', { detail: { saveState: this.hasUnsavedChanges } });
                    window.dispatchEvent(afterFrameEvent);
                }, this.transitionDuration + 50);

            }, i * (this.transitionDuration + 200));
        }

        const afterTimelineEvent = new CustomEvent('afterTimelineTransition', { detail: { saveState: this.hasUnsavedChanges } });
       // window.dispatchEvent(afterTimelineEvent);
    }
    ...
    maybeSaveStateChanged() {
        // check if save state has changed
        if (this.hasUnsavedChanges === this._saveStateLast) {
            return;
        }
        
        // new event
        const event = new CustomEvent('timelineSaveStateChanged', { detail: { saveState: this.hasUnsavedChanges } });
        // dispatch event
        window.dispatchEvent(event);

        // update last save state
        this._saveStateLast = this.hasUnsavedChanges;
...
class MeshGradientFrame {
    ...
    setTimelineFrameEl(timelineFrameEl) {
        this.timelineFrameEl = timelineFrameEl;
    }

    getTimelineFrameEl() {
        return this.timelineFrameEl;
    }
